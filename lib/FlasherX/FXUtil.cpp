...
//******************************************************************************
// update_firmware_noprompt() — same as update_firmware() but skips the
// interactive line-count confirmation prompt. Designed for UART-driven OTA
// where the host has already validated the .hex (sha256, etc.) and just wants
// us to apply it. On EOF + all sanity checks pass, calls flash_move().
//******************************************************************************
                               ...
                               uint32_t buffer_addr, uint32_t buffer_size )
{
  ...
  out->printf( "FX: reading hex lines (no-prompt mode)...\n" );
  ...
  while (!hex.eof)  {
    ...
    if (parse_hex_line( (const char*)line, hex.data, &hex.addr, &hex.num, &hex.code ) == 0) {
      ...
      if (hex.max > (FLASH_BASE_ADDR + buffer_size)) {
        ...
        int error = flash_write_block( addr, hex.data, hex.num );
        if (error) {
          out->printf( "FX: abort - error %d in flash_write_block()\n", error );
          return;
        }
      }
    }
  ...
  if (check_flash_id( buffer_addr, hex.max - hex.min )) {
    out->printf( "FX: new code contains correct target ID %s\n", FLASH_ID );
  }
  else {
  ...
  out->flush();
  // No return from flash_move(): copies new program into place + reboots.
  flash_move( FLASH_BASE_ADDR, buffer_addr, hex.max - hex.min );
  REBOOT;
}
